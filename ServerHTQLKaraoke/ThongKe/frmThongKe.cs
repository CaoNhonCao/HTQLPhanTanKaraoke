...
namespace ServerHTQLKaraoke.ThongKe
{
    public partial class frmThongKe : Form
    {
        ...
        private void LoadChartData(int month, int year)
        {
            ...
            decimal totalChiPhiBaoTri = GetTotalChiPhiBaoTri(month, year);
            ...
            decimal totalChiTieu = totalChiPhiKhac + totalNhapHang + totalChiPhiBaoTri + totalLuongNhanVien;
            decimal profit = totalThuNhap - totalChiTieu;
            tongChiPhi = totalChiTieu;
            tongDoanhThu = totalThuNhap;
            loiNhuan = profit;
            // Cập nhật biểu đồ
            UpdateChart(totalChiPhiKhac, totalNhapHang, totalChiPhiBaoTri, totalLuongNhanVien, totalThuNhap, profit);
        }
        private void UpdateChart(decimal totalChiPhiKhac, decimal totalNhapHang, decimal totalChiPhiBaoTri, decimal totalLuongNhanVien, decimal totalThuNhap, decimal profit)
        {
            ...
            seriesChiTieu.Points.AddXY("C", totalChiPhiBaoTri);
            seriesChiTieu.Points.AddXY("D", totalLuongNhanVien);
            ...
            seriesThuNhap.Points.AddXY("E", totalThuNhap);  // Thu nhập (E)
            ...
            seriesLoiNhuan.Points.AddXY("F", profit);  // Lợi nhuận (F)
            chartLoiNhuan.Series.Add(seriesLoiNhuan);

            // Định dạng giá trị trên biểu đồ với VND
            FormatChartLabels(chartChiTieu, totalChiPhiKhac, totalNhapHang, totalChiPhiBaoTri, totalLuongNhanVien);
        ...
        private void AddExplanationLabels()
        {
            ...
            labelExplanation.Text = "A: Chi Phí Khác\n" +
                                    ...
                                    "D: Lương Nhân Viên\n" +
                                    "E: Thu Nhập (Hóa Đơn)\n" +
                                    "F: Lợi Nhuận (Thu Nhập - Tổng Chi Tiêu)";
        ...
        private decimal GetTotalThuNhap(int month, int year)
        {
            decimal total = 0;
            string query = @"
        SELECT SUM(TRY_CAST(REPLACE(ThanhToan, '.', '') AS DECIMAL(18, 2))) 
        FROM HoaDon
        ...
        private decimal GetTotalChiPhiBaoTri(int month, int year)
        {
            decimal total = 0;
            string query = @"SELECT SUM(ChiPhiBaoTri)
                     FROM LichSuBaoTri
                     WHERE MONTH(NgayBaoTri) = @Month AND YEAR(NgayBaoTri) = @Year";

            // Thực hiện truy vấn và lấy tổng chi phí bảo trì
            using (var conn = new SqlConnection(connection))
            {
                conn.Open();
                using (var command = new SqlCommand(query, conn))
