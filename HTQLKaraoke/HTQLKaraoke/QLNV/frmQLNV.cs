                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            btnEmployee.Size = new Size(175, 175);
                            ...
                            try
                            {
                                if (chucVu == "Phục vụ")
                                {
                                    btnEmployee.Image = Image.FromFile(@"\HTQLKaraoke\HTQLKaraoke\Image\phucvu.gif");
                                }
                                else if (chucVu == "Lễ tân")
                                {
                                    btnEmployee.Image = Image.FromFile(@"\HTQLKaraoke\HTQLKaraoke\Image\letan.gif");
                                }
                                else if (chucVu == "Bảo vệ")
                                {
                                    btnEmployee.Image = Image.FromFile(@"\HTQLKaraoke\HTQLKaraoke\Image\baove.gif");
                                    btnEmployee.ImageAlign = ContentAlignment.MiddleCenter;
                                }
                                else
                                {
                                    // Mặc định cho các chức vụ khác nếu không có hình ảnh cụ thể
                                    btnEmployee.Image = Image.FromFile(@"\HTQLKaraoke\HTQLKaraoke\Image\default.gif");
                                }
                            }
